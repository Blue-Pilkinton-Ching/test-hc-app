    ...
    match op.flattened::<EntryTypes, LinkTypes>()? {
        FlatOp::StoreEntry(store_entry) => match store_entry {
            OpEntry::CreateEntry { app_entry, action } => match app_entry {
                EntryTypes::TextEntry(text_entry) => {
                    validate_create_text_entry(EntryCreationAction::Create(action), text_entry)
                }
            },
            OpEntry::UpdateEntry {
                app_entry, action, ..
                ...
                EntryTypes::TextEntry(text_entry) => {
                    validate_create_text_entry(EntryCreationAction::Update(action), text_entry)
                }
            },
            ...
            OpUpdate::Entry { app_entry, action } => {
                ...
                match app_entry {
                    EntryTypes::TextEntry(text_entry) => {
                        ...
                        let original_text_entry = match TextEntry::try_from(original_app_entry) {
                            Ok(entry) => entry,
                            Err(e) => {
                                return Ok(ValidateCallbackResult::Invalid(format!(
                                    "Expected to get TextEntry from Record: {e:?}"
                                )));
                            }
                        };
                        validate_update_text_entry(
                            action,
                            text_entry,
        ...
        FlatOp::RegisterDelete(delete_entry) => {
            ...
            match original_app_entry {
                EntryTypes::TextEntry(original_text_entry) => validate_delete_text_entry(
        ...
        FlatOp::RegisterCreateLink {
            ...
            LinkTypes::TextEntryUpdates => {
                validate_create_link_text_entry_updates(action, base_address, target_address, tag)
            }
        },
        FlatOp::RegisterDeleteLink {
            ...
            LinkTypes::TextEntryUpdates => validate_delete_link_text_entry_updates(
                action,
                original_action,
                base_address,
                target_address,
        ...
        FlatOp::StoreRecord(store_record) => {
            match store_record {
                ...
                OpRecord::UpdateEntry {
                    ...
                    match app_entry {
                        EntryTypes::TextEntry(text_entry) => {
                            let result = validate_create_text_entry(
                                EntryCreationAction::Update(action.clone()),
                                text_entry.clone(),
                            )?;
                            if let ValidateCallbackResult::Valid = result {
                                let original_text_entry: Option<TextEntry> = original_record
                                    .entry()
                                    .to_app_option()
                                    .map_err(|e| wasm_error!(e))?;
                                let original_text_entry = match original_text_entry {
                                    Some(text_entry) => text_entry,
                                    None => {
                                        return Ok(
                ...
                OpRecord::DeleteEntry {
                    ...
                    match original_app_entry {
                        EntryTypes::TextEntry(original_text_entry) => {
                            validate_delete_text_entry(action, original_action, original_text_entry)
                        }
                    }
                }
                ...
                OpRecord::CreateLink {
                    ...
                    LinkTypes::TextEntryUpdates => validate_create_link_text_entry_updates(
                        action,
                        base_address,
                        target_address,
                        tag,
